        ...
        <div class="tpl-block">
            <div class="am-g">
                <div class="am-u-sm-12"  style="text-align: right">
                    ...
                    <form id="test_form" class="am-form">
                        <button id="connect" class="am-btn" type="submit">Connect</button>
                        <button id="disconnect" class="am-btn-danger" type="submit" disabled="disabled">Disconnect</button>
                        <button id="send" class="am-btn" type="submit">Send</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    function connect() {
        // 新建SockjS对象，并设置服务端的连接点（/gs-guide-websocket）
        // 这里的连接点由服务端提供。
        ...
        stompClient.connect({"id": "header"}, function (frame) {
            ...
            // subscribe()方法的第一个参数是注册客户端地址，注意前戳必须是/teacher-websocket开头
            // 因为在前面服务端已经配置了目的地前戳。与@SendTo中的地址对应。
            stompClient.subscribe('/teacher-websocket/accept', function (message) {
    ...
    function sendName() {
        ...
        // @MessageMapping地址并且加了指定的/app前戳，
        // 第二个参数为header头部信息
        // 第三个是发送的消息内容。
        stompClient.send("/websocket/teacher/send", {}, "teacher: " + ${teacher.getAccount()});
    }
    (function () {
