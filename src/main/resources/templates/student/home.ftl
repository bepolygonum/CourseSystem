    ...
    <script>
        ...
        function connect() {
            ...
            stompClient.connect({"id": "header"}, function (frame) {
                ...
                // subscribe()方法的第一个参数是注册客户端地址，注意前戳必须是/topic开头
                // 因为在前面服务端已经配置了目的地前戳。与@SendTo中的地址对应。
                stompClient.subscribe('/student-websocket/accept', function (message) {
        ...
        function sendName() {
            ...
            // 第二个参数为header头部信息
            // 第三个是发送的消息内容。
            stompClient.send("/websocket/student/send/", {}, "student: " + ${student.getAccount()});
        }
        (function () {
